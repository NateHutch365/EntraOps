# Requirements Archive: v1.1 Pre-Apply Intelligence
...
- [x] **DASH-01**: Dashboard displays both applied tier counts and computed tier counts side-by-side per tier (ControlPlane / ManagementPlane / UserAccess)
- [x] **DASH-02**: Computed tier is derived from the lowest `AdminTierLevel` number in an object's `Classification[]` array
- [x] **DASH-03**: Dashboard visually distinguishes applied vs. computed counts with clear "Applied" / "Suggested" labels

### Object Browser Computed Tier

- [x] **OBJ-01**: When `ObjectAdminTierLevelName` is "Unclassified", the Tier column shows the computed tier instead
- [x] **OBJ-02**: Computed tier badge is visually distinct from an applied tier (dashed outline style)
- [x] **OBJ-03**: When an applied tier is set, existing Tier column behaviour is unchanged
...
| OBJ-01 | Phase 7 | ✅ Complete |
| OBJ-02 | Phase 7 | ✅ Complete |
| OBJ-03 | Phase 7 | ✅ Complete |
